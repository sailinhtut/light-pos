import { Children, RefObject, useContext, useEffect, useRef, useState } from "react";
...
export function OrderActiveDetailPage() {
  ...
  const loadData = async ({ updateConfig }: { updateConfig: boolean }) => {
    ...
    if (orderId) {
      const data = await OrderActiveService.getOrderHistory(orderId);
      setOrder(data);
    }
  ...
  const processCheckOut = async () => {
    ...
    setCheckingOut(true);
    await OrderActiveService.deleteOrderHistory(order!.orderId);
                        ...
                        <CustomItemDialog
                          onCreated={async (cartItem) => {
                            if (order) {
                              ...
                              order.amount = order.getAmount();
                              order.total = order.calculateTotal();
                              await OrderActiveService.updateOrderHistory(order);
            ...
            <div className="mt-3 w-1/2 px-10">
                    ...
                    {offlineMode && (
                      <TableCell className="py-0.5 overflow-clip">
                        <ValueInputDialog
                          ...
                          description=""
                          initValue=""
                          title="Enter Casher Name"
                          onSubmit={async (value) => {
                            if (value) {
                              localStorage.setItem("offline_casher_name", value);
                              order.casherName = value;
                              await OrderActiveService.updateOrderHistory(order);
                              await loadData({ updateConfig: true });
                            }
                          }}
              ...
              {updating ? (
                ...
                <div className="mt-5 flex gap-x-4 border-b pb-20">
                  <PrintOrder order={order} creditBookId={selectedCreditBook?.creditBookId}>
...
export function CreditBookSelectDialog({
  onSelected,
  children
}: {
  onSelected: (book: Creditbook | undefined) => void;
  children?: React.ReactNode;
}) {
  const [selectedBook, setSelectedBook] = useState<Creditbook | undefined>();
  const [creditBooks, setCreditBookds] = useState<Creditbook[]>([]);
  const [loading, setLoading] = useState(false);
      ...
      <DialogContent hideCloseButton={true}>
        <DialogHeader>
          <DialogTitle className="w-full flex flex-row justify-between items-center ">
            <span className="flex flex-row items-center">Select Credit Book</span>
            <CreditbookDialog
              title="Add Creditbook"
              actionTitle="Add"
              onCreated={async (creditBook) => {
                setLoading(true);
                await CreditbookService.addCreditbook(creditBook);
                toast({
        ...
        <div className="flex flex-col h-[400px] ">
          {loading && <LoadingWidget />}
          {!loading && !creditBooks && <p>No Credit Book</p>}
          {!loading && creditBooks && (
            <ScrollArea className="h-[400px] pr-3">
              {creditBooks.map((book, index) => (
                <Card key={index} className="mb-3" onClick={() => setSelectedBook(book)}>
                  <CardHeader className="p-3 flex flex-row justify-between items-center">
                    <div>
                      ...
                      <CardDescription>
                        Amount - {formatNumberWithCommas(book.creditAmount)}
                      </CardDescription>
                    </div>
                    <motion.div
                      initial={{
                      ...
                      animate={{
                        scale:
                          selectedBook && selectedBook.creditBookId === book.creditBookId ? 1 : 0
                      }}
                    >
                      <div className="origin-bottom size-8 rounded-full bg-green-500 flex justify-center items-center border-slate-300 drop-shadow">
...
export function CustomItemDialog({
  ...
  const submit = (data: FieldValues) => {
    ...
    const newCartItem = CartItem.fromJson({
      itemId: `customItem-${uniqueId(4)}`,
      itemName: data.name,
      itemDescription: data.description ?? "",
      purchasedPrice: parseNumber(data.purchasedPrice) ?? 0,
      price: parseNumber(data.salePrice) ?? 0,
        ...
        <Form {...priceVariantForm}>
          ...
          >
            <DialogHeader>
              ...
              <DialogDescription>
                The item you create here is just for this order. It will not save permanently.
              </DialogDescription>
            </DialogHeader>
            <FormField
              name="name"
            ...
            <FormField
              name="purchasedPrice"
              rules={{
                required: "Purchased Price is requried",
            ...
            <FormField
              name="salePrice"
              rules={{
                required: "Sale Price is requried",
                validate: (value) => !isNaN(value) || "Invalid Number"
              }}
              render={({ field }) => (
                <FormItem>
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      >
        <DialogHeader>
          <DialogTitle className="w-full flex flex-row justify-between items-center ">
            <span className="flex flex-row items-center">{item.itemName}</span>
          </DialogTitle>
          <DialogDescription>{item.itemDescription}</DialogDescription>
        </DialogHeader>

        <div className="pb-5 flex flex-col gap-3">
          <p className="text-sm">Item ID - {item.itemId}</p>
          <p className="text-sm">Taken Quantity - {item.quantity}</p>
          <p className="text-sm">
            Price - {formatNumberWithCommas(item.price)} {currency}
          </p>

          <p className="text-sm">
            Total Cost - {formatNumberWithCommas(item.getTotal())} {currency}
          </p>

          <p className="text-sm">
            Total Purchased - {formatNumberWithCommas(item.getTotalPurchasedPrice())} {currency}
          </p>

          <p className="text-sm">
            Profit - {formatNumberWithCommas(item.getTotal() - item.getTotalPurchasedPrice())}{" "}
            {currency}
          </p>
        </div>
